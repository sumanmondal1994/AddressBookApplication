-- Seed data for Address Book Application
-- Uses MERGE to insert only if data doesn't exist (prevents duplicates on restart)

-- Insert Address Books (MERGE: insert if not exists, skip if exists)
MERGE INTO addressbooks (id, name, description, created_at, updated_at) KEY(id) VALUES
...
MERGE INTO contacts (id, name, phone_number, address_book_id, created_at) KEY(id) VALUES
...
-- Set addressbooks sequence to start after ID 10
ALTER TABLE addressbooks ALTER COLUMN id RESTART WITH 11;

-- Set contacts sequence to start after ID 100
ALTER TABLE contacts ALTER COLUMN id RESTART WITH 101;
